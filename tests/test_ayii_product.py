...
def test_trigger_and_cancel_oracle_requests(
    ...
    with brownie.reverts('ERROR:AYI-011:ORACLE_ALREADY_TRIGGERED'):
        txRepeat = product.triggerOracle(processId, {'from': insurer})
        print(txRepeat.info())
    ...
    print('oracle request triggered a 2nd time'.format(tx.info()))
    assert requestId2 == 1

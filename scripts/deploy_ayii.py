...
def help():
    ...
    print(
        "from scripts.deploy_ayii import stakeholders_accounts_ganache, check_funds, amend_funds, deploy, deploy_product_riskpool, from_registry, from_component, verify_deploy"
    )
    print()
    ...
    print()
    print(
        "(instance, product, oracle, riskpool) = deploy_product_riskpool(registry_address, a, token, collateralization_level)"
    )
...
def check_funds(stakeholders_accounts, erc20_token):
    ...
    for accountName, requiredAmount in REQUIRED_FUNDS.items():
        if a[accountName].balance() >= REQUIRED_FUNDS[accountName]:
            ...
            print(
                "{} needs {} but has {}".format(
                    accountName, REQUIRED_FUNDS[accountName], a[accountName].balance()
                )
            )
    ...
    if fundsMissing > 0:
        ...
        if (
            a[INSTANCE_OPERATOR].balance()
            >= REQUIRED_FUNDS[INSTANCE_OPERATOR] + fundsMissing
        ):
            print(
                "{} sufficiently funded with native token to cover missing funds".format(
                    INSTANCE_OPERATOR
                )
            )
        else:
            additionalFunds = (
                REQUIRED_FUNDS[INSTANCE_OPERATOR]
                + fundsMissing
                - a[INSTANCE_OPERATOR].balance()
            )
            print(
                "{} needs additional funding of {} ({} ETH) with native token to cover missing funds".format(
                    INSTANCE_OPERATOR, additionalFunds, additionalFunds / 10**18
                )
            )
    else:
        native_token_success = True
...
def check_erc20_funds(a, erc20_token):
    if erc20_token.balanceOf(a[INSTANCE_OPERATOR]) >= INITIAL_ERC20_BUNDLE_FUNDING:
        ...
        print(
            "{} needs additional ERC20 funding of {} to cover missing funds".format(
                INSTANCE_OPERATOR,
                INITIAL_ERC20_BUNDLE_FUNDING
                - erc20_token.balanceOf(a[INSTANCE_OPERATOR]),
            )
        )
...
def _get_balances_delta(balances_before, balances_after):
    ...
    for accountName, account in balances_before.items():
        balance_delta[accountName] = (
            balances_before[accountName] - balances_after[accountName]
        )
...
def deploy_setup_including_token(
    stakeholders_accounts, erc20_token, publishSource=False
):
...
def verify_deploy(
    ...
    (instance, product, oracle, riskpool, componentController) = from_registry(
        registry_address, riskpoolId=riskpoolId, oracleId=oracleId, productId=productId
    )
    ...
    verify_element(
        "RiskpoolBalance",
        instanceService.getBalance(riskpoolId),
        erc20_token.balanceOf(riskpoolWallet),
    )
    ...
    print(
        "InstanceWalletBalance {:.2f}".format(
            erc20_token.balanceOf(instanceService.getInstanceWallet())
            / 10 ** erc20_token.decimals()
        )
    )
    print(
        "RiskpoolWalletTVL {:.2f}".format(
            instanceService.getTotalValueLocked(riskpoolId)
            / 10 ** erc20_token.decimals()
        )
    )
    print(
        "RiskpoolWalletCapacity {:.2f}".format(
            instanceService.getCapacity(riskpoolId) / 10 ** erc20_token.decimals()
        )
    )
    print(
        "RiskpoolWalletBalance {:.2f}".format(
            erc20_token.balanceOf(instanceService.getRiskpoolWallet(riskpoolId))
            / 10 ** erc20_token.decimals()
        )
    )
...
def verify_element(element, value, expected_value):
...
def deploy_product_with_oracle_riskpool(
    ...
    print(
        "====== getting instance from registry address {} ======".format(
            registry_address
        )
    )
    ...
    erc20Token.approve(
        instance.getTreasury().address, initial_funding, {"from": investor}
    )
...
def deploy(stakeholders_accounts, erc20_token, publishSource=False):
    ...
    print(
        "1) investor {} funding (transfer/approve) with {} token for erc20 {}".format(
    ...
    print(
        "2) riskpool wallet {} approval for instance treasury {}".format(
            riskpoolWallet, instance.getTreasury()
        )
    )

    erc20Token.approve(
        instance.getTreasury(), bundleInitialFunding, {"from": riskpoolWallet}
    )

    print("3) riskpool bundle creation by investor {}".format(investor))
    ...
    print("4) risk creation (2x) by insurer {}".format(insurer))
    ...
    print(
        "5) customer {} funding (transfer/approve) with {} token for erc20 {}".format(
    ...
    print(
        "6) policy creation (2x) for customers {}, {} by insurer {}".format(
    ...
    deploy_result = {
        ...
        INSTANCE_OPERATOR_SERVICE: contract_from_address(
            InstanceOperatorService, instanceOperatorService
        ),
        COMPONENT_OWNER_SERVICE: contract_from_address(
            ComponentOwnerService, componentOwnerService
        ),
...
def from_registry(
    registryAddress, productId=0, oracleId=0, riskpoolId=0, verbose=False
):
    ...
    if products >= 1:
        ...
        if product.getType() != 1:
            ...
            if verbose:
                print(
                    "component (type={}) with id {} is not product".format(
...
def dry_run_create_risks(product, insurer):
    ...
    aez = [22, 23, 26, 29, 30, 32, 34, 38, 39, 40]

    aph = [2.28, 2.42, 2.14, 2.01, 3.03, 3.05, 2.38, 1.84, 2.60, 2.30]

    print("project, aez, crop, trigger, exit, tsi, aph, riskId")
    for i in range(len(aez)):
        riskId = create_risk(
            product, insurer, project, aez[i], crop, trigger, exit_, tsi, aph[i]
        )

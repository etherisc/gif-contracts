...
def h2s(hex: str) -> str:
    return Web3.to_text(hex).split("\x00")[-1]


def h2sLeft(hex: str) -> str:
    return Web3.to_text(hex).split("\x00")[0]
...
def get_account(mnemonic: str, account_offset: int) -> Account:
    return accounts.from_mnemonic(mnemonic, count=1, offset=account_offset)
...
def deployGifModule(controllerClass, storageClass, registry, owner, publishSource):
    controller = controllerClass.deploy(
        registry.address, {"from": owner}, publish_source=publishSource
    )
...
def deployGifToken(tokenName, tokenClass, registry, owner, publishSource):
    print("token {} deploy".format(tokenName))
    token = tokenClass.deploy({"from": owner}, publish_source=publishSource)
...
def deployGifModuleV2(moduleName, controllerClass, registry, owner, publishSource):
    print("module {} deploy controller".format(moduleName))
    controller = controllerClass.deploy({"from": owner}, publish_source=publishSource)

    encoded_initializer = encode_function_data(
        registry.address, initializer=controller.initialize
    )
...
def deployGifService(serviceClass, registry, owner, publishSource):
...
def deployGifServiceV2(serviceName, serviceClass, registry, owner, publishSource):
    service = serviceClass.deploy(
        registry.address, {"from": owner}, publish_source=publishSource
    )

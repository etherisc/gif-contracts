...
def fund_riskpool(
    ...
    if createBundle:
        tx = riskpool.createBundle(applicationFilter, amount, {"from": bundleOwner})
...
def fund_customer(
    instance: GifInstance, owner: Account, account: Account, coin, amount: int
):
...
def apply_for_policy(
    ...
    tx = product.applyForPolicy(
        premium, sumInsured, metaData, applicationData, {"from": customer}
    )

...
contract PoolController is IPool, CoreController {
    ...
    uint256 public constant COLLATERALIZATION_LEVEL_CAP =
        2 * FULL_COLLATERALIZATION_LEVEL;

    uint256 public constant DEFAULT_MAX_NUMBER_OF_ACTIVE_BUNDLES = 1;

    mapping(bytes32 /* processId */ => uint256 /* collateralAmount*/)
        private _collateralAmount;

    mapping(uint256 /* productId */ => uint256 /* riskpoolId */)
        private _riskpoolIdForProductId;

    mapping(uint256 /* riskpoolId */ => IPool.Pool) private _riskpools;

    mapping(uint256 /* riskpoolId */ => uint256 /* maxmimumNumberOfActiveBundles */)
        private _maxmimumNumberOfActiveBundlesForRiskpoolId;

    mapping(uint256 /* riskpoolId */ => EnumerableSet.UintSet /* active bundle id set */)
        private _activeBundleIdsForRiskpoolId;
    ...
    modifier onlyActivePool(uint256 riskpoolId) {
        require(
            _component.getComponentState(riskpoolId) ==
    ...
    function registerRiskpool(
        ...
        _maxmimumNumberOfActiveBundlesForRiskpoolId[
            riskpoolId
        ...
        require(
            collateralizationLevel <= COLLATERALIZATION_LEVEL_CAP,
            "ERROR:POL-008:COLLATERALIZATION_lEVEl_TOO_HIGH"
        );
        require(
            sumOfSumInsuredCap > 0,
    ...
    function setRiskpoolForProduct(
        uint256 productId,
        uint256 riskpoolId
    ) external override onlyInstanceOperatorService {
        ...
        require(
            _riskpoolIdForProductId[productId] == 0,
    ...
    function fund(
        uint256 riskpoolId,
        uint256 amount
    ) external onlyRiskpoolService onlyActivePool(riskpoolId) {
        IPool.Pool storage pool = _riskpools[riskpoolId];
        pool.capital += amount;
        pool.balance += amount;
        pool.updatedAt = block.timestamp;
        ...
        returns (bool success)
    {
        ...
        IPolicy.Application memory application = _policy.getApplication(
            processId
        );
        require(
        ...
        emit LogRiskpoolRequiredCollateral(
            processId,
            sumInsuredAmount,
            collateralAmount
        );
        ...
        if (success) {
            ...
            emit LogRiskpoolCollateralizationSucceeded(
                riskpoolId,
                processId,
                sumInsuredAmount
            );
    ...
    function calculateCollateral(
        ...
        uint256 collateralization = getRiskpool(riskpoolId)
            .collateralizationLevel;
        ...
        if (collateralization == FULL_COLLATERALIZATION_LEVEL) {
            ...
            collateralAmount =
                (collateralization * sumInsuredAmount) /
    ...
    function release(
        bytes32 processId
    ) external override onlyPolicyFlow("Pool") {
        ...
        uint256 riskpoolId = _riskpoolIdForProductId[metadata.productId];
        IPool.Pool storage pool = _riskpools[riskpoolId];
        uint256 remainingCollateralAmount = _collateralAmount[processId] -
            policy.payoutAmount;
        ...
        emit LogRiskpoolCollateralReleased(
            riskpoolId,
            processId,
    ...
    function setMaximumNumberOfActiveBundles(
        uint256 riskpoolId,
        uint256 maxNumberOfActiveBundles
    ) external onlyRiskpoolService {
        require(
            maxNumberOfActiveBundles > 0,
            "ERROR:POL-032:MAX_NUMBER_OF_ACTIVE_BUNDLES_INVALID"
        );
        _maxmimumNumberOfActiveBundlesForRiskpoolId[
            riskpoolId
        ] = maxNumberOfActiveBundles;
    }

    function getMaximumNumberOfActiveBundles(
        uint256 riskpoolId
    ) public view returns (uint256 maximumNumberOfActiveBundles) {
        return _maxmimumNumberOfActiveBundlesForRiskpoolId[riskpoolId];
    }

    function riskpools() external view returns (uint256 idx) {
        return _riskpoolIds.length;
    }

    function getRiskpool(
        uint256 riskpoolId
    ) public view returns (IPool.Pool memory riskPool) {
        riskPool = _riskpools[riskpoolId];
        require(
            riskPool.createdAt > 0,
            "ERROR:POL-040:RISKPOOL_NOT_REGISTERED"
        );
    }

    function getRiskPoolForProduct(
        uint256 productId
    ) external view returns (uint256 riskpoolId) {
        return _riskpoolIdForProductId[productId];
    }

    function activeBundles(
        uint256 riskpoolId
    ) external view returns (uint256 numberOfActiveBundles) {
    ...
    function getActiveBundleId(
        uint256 riskpoolId,
        uint256 bundleIdx
    ) external view returns (uint256 bundleId) {
        require(
            bundleIdx <
                EnumerableSet.length(_activeBundleIdsForRiskpoolId[riskpoolId]),
            "ERROR:POL-041:BUNDLE_IDX_TOO_LARGE"
        );

        return
            EnumerableSet.at(
                _activeBundleIdsForRiskpoolId[riskpoolId],
                bundleIdx
            );
    }

    function addBundleIdToActiveSet(
        uint256 riskpoolId,
        uint256 bundleId
    ) external onlyRiskpoolService {
        require(
        ...
        require(
            EnumerableSet.length(_activeBundleIdsForRiskpoolId[riskpoolId]) <
                _maxmimumNumberOfActiveBundlesForRiskpoolId[riskpoolId],
    ...
    function removeBundleIdFromActiveSet(
        uint256 riskpoolId,
        uint256 bundleId
    ) external onlyRiskpoolService {
        require(
    ...
    function _getRiskpoolComponent(
        IPolicy.Metadata memory metadata
    ) internal view returns (IRiskpool riskpool) {
        uint256 riskpoolId = _riskpoolIdForProductId[metadata.productId];

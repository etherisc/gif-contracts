...
contract RegistryController is IRegistry, CoreController {
    ...
    mapping(bytes32 /* release */ => mapping(bytes32 /* contract name */ => address /* contract address */))
        public _contracts;
    mapping(bytes32 /* release */ => uint256 /* number of contracts in release */)
        public _contractsInRelease;
    mapping(bytes32 /* release */ => EnumerableSet.Bytes32Set /* contract names */)
        private _contractNames;
    ...
    ) external view override returns (bool _senderMatches) {
        _senderMatches = (sender ==
            _getContractInRelease(release, _contractName));
    }

    /**
     * @dev get current release
     */
    function getRelease() external view override returns (bytes32 _release) {
        _release = release;
    }
    ...
    function getContract(
        bytes32 _contractName
    ) public view override returns (address _addr) {
    ...
    function getContractInRelease(
        bytes32 _release,
        bytes32 _contractName
    ) external view override returns (address _addr) {
    ...
    function contracts()
        ...
        view
        override
        returns (uint256 _numberOfContracts)
    {
    ...
    function contractName(
        uint256 idx
    ) external view override returns (bytes32 _contractName) {
     ...
     * @dev Get contract's address in certain release
     */
    function _getContractInRelease(
        bytes32 _release,
        bytes32 _contractName
    ) internal view returns (address _addr) {
    ...
    function _registerInRelease(
        ...
        require(
            _contractsInRelease[_release] > 0 || isNewRelease,
        ...
        require(
            (
                ...
                (_contractName == "InstanceOperatorService" &&
                    _contracts[_release][_contractName] == _msgSender()),
        ...
        require(
            _contractsInRelease[_release] ==
                EnumerableSet.length(_contractNames[_release]),
